...
class NewDetailController extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('NewDetailAD/index'); // danh sach
	...
	{	
		$this->load->view('TemplateAD/header');
		$this->load->view('TemplateAD/navbar');
		$this->load->view('NewDetailAD/create');
		$this->load->view('TemplateAD/footer');
	...
	{	
        ...
        $data['NewDetail'] = $this->NewDetailModel->selectNewDetailList();
		$this->load->view('NewDetailAD/list', $data);
		$this->load->view('TemplateAD/footer');
...
/* -------------------------------------------------------------------------- */
/*                                Thêm tin tức                                */
	public function  formNewDetail(){
		$this->form_validation->set_rules('newDetailName', 'Tên tin tức', 'trim|required', ['required' => 'Vui lòng nhập %s']);
		$this->form_validation->set_rules('newDetailDesc','mô tả', 'trim|required', ['required' => 'Vui lòng nhập %s ']);
		$this->form_validation->set_rules('newDetailTime','Giá', 'trim|required', ['required' => 'Vui lòng nhập %s ']);
		if ($this->form_validation->run() == true) 
				{
					// upload file image
						$ori_filename = $_FILES['newDetailIMG']['name'];
						$new_name = time()."".str_replace(' ','-', $ori_filename);
						$config = [
							'upload_path' => './uploads/NewDetail', // xem đường dẫn file có đúng không
						...
						if ( ! $this->upload->do_upload('newDetailIMG'))
                		{
                       		 $error = array('error' => $this->upload->display_errors());
								$this->load->view('TemplateAD/header');
								$this->load->view('TemplateAD/navbar');
								$this->load->view('NewDetailAD/create',$error);
								$this->load->view('TemplateAD/footer');
                     
               			}else{
							// hàm upload product
							$newDetailAD_filename = $this->upload->data('file_name');
							//mảng data insert
							$data = [
								// các cột database------------lấy từ các input 
								'newsDetailHeading' => $this->input->post('newDetailName'),
								'descriptionDetail' => $this->input->post('newDetailDesc'),
								'create_at' => $this->input->post('newDetailTime'),
								'newsDetailIMG' => $newDetailAD_filename

							];
							// hàm gọi 
							$this->load->model('NewDetailModel');
							$this->NewDetailModel->insertNewDetail($data); // kiểm tra đúng hàm sử dụng hay không?
							$this->session->set_flashdata('success','thêm thành công');
							redirect(base_url('NewDetail/create'));
						}
				}
				else
				{
